...
const contactForm=document.getElementById('contactForm');const formStatus=document.getElementById('formStatus');contactForm.addEventListener('submit',(e)=>{const name=document.getElementById('formName').value.trim();const email=document.getElementById('formEmail').value.trim();const message=document.getElementById('formMessage').value.trim();const errs=[];if(!name)errs.push('Name');if(!email)errs.push('Email');if(!message)errs.push('Message');if(errs.length){e.preventDefault();formStatus.innerHTML='&#10022; Missing: '+errs.join(', ');formStatus.className='form-status error';}});
